...
def show_label(text: str, x: init, y: init, font_name: str='Arial', color=(255, 255, 255)):
    '''
    выводит на игровую сцену window
    '''
    font.init()
...
class Enemy(GameSprite):
    ...
    def set_hp(self):
        ...
        if self.hp in (3, 4, 5):
            ...
            x , y = self.rect.x, self.rect.y
            self.image = image.load(self.image_name)
            self.image = transform.scale(self.image, (SPRITE_SIZE//2, SPRITE_SIZE))
            self.rect = self.image.get_rect()
            self.rect.x = x
            self.rect.y = y                             
...
player = Player('rocket.png', 7, SCREEN_SIZE[0]//2,SCREEN_SIZE[1] - SPRITE_SIZE)
player.last_shoot = 0
...
while True:
    clock.tick(FPS)
    window.blit(pic, (0,0))
    show_label(str(int(end - get_time())), SCREEN_SIZE[0]//2, SCREEN_SIZE[1]//2 -20)
...
while game:
    ...
    if finish == False:
        ...
        if killed_counter.counter >= 50:
            show_label('Победа',  SCREEN_SIZE[0]//2 - 40 , SCREEN_SIZE[0]//2 - 20)
            finish = True
